...
def createDataframe():
  cardatanames = os.listdir('../cardata/output')

  inputdatanames = os.listdir('../cardata/input')
  ...
  for (outnames, innames) in zip(cardatanames, inputdatanames):
    df = pd.read_csv( f'../cardata/output/{outnames}')

    df1 = pd.read_csv( f'../cardata/input/{innames}')
    ...
    a = (abs(df['SlipAngle_FR'].abs().mean() - df['SlipAngle_RR'].abs().mean())  + abs(df['SlipAngle_FL'].abs().mean() - df['SlipAngle_RL'].abs().mean()))/2
    values.append(a)
